...
| (reference) Haralick oct_zero | Haralick zero rule + GDSD **octant direction** (ρ2.0/σ2.8) | the reference internal pipeline reports oct_zero as its top scorer (their own split-normalized pipeline, not this protocol); direction quantization should be ~free per the Rayleigh prediction | `haralick_facet.py direction="octant"` | 0.5962 | reference — ≈ continuous Haralick (0.5976) under official protocol |
